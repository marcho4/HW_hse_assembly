.data
	epsilon: .float 0.000005 
	sub_msg: .asciz "Subtracting\n"
	add_msg: .asciz "Adding\n"
...
.text
	calculate_arctan: #signature (x: float (should be in fa0)) returns float
		...
		fcvt.s.w fs0, t0 # fs0 - сумма ряда
		
		fmv.s ft1, fa0 # ft1 - x
		
		# ft0 = n, ft1 = x, fs0 = сумма, fs2 - epsilon
	...
	calculate_curr:
		...
		fcvt.w.s t0, ft0
		andi t5, t0, 1
		beqz t5, add_to_sum     # Если младший бит = 0, число четное
    		j subtract_from_sum
	...
	subtract_from_sum:
		fsub.s fs0, fs0, fa0 # add fa0 to summ
		print(sub_msg)
		j check_accuracy
